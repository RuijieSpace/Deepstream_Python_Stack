...
"""
DeepStream 8.0 三模型串行推理（对比基准）- 支持多种编码格式
pgie1 → pgie2 → pgie3 顺序执行
"""
...
OUTPUT_VIDEO_PATH = "/workspace/3models_serial_output.mp4"
...
def osd_sink_pad_buffer_probe(pad, info, u_data):
    ...
    while l_frame is not None:
        ...
        params_fps.display_text = f"SERIAL-3M | FPS: {current_fps:.1f} | Frame: {frame_number}"
        ...
        if frame_number % 30 == 0:
            ...
            print(f"[SERIAL] Frame {frame_number:4d} | FPS: {current_fps:5.1f} | M1:{m1:2d} M2:{m2:2d} M3:{m3:2d}")
...
def main(args):
    ...
    print("\n" + "="*80)
    print("DeepStream 8.0 - 3-Model SERIAL Inference (Multi-Format)")
    print("="*80)
    ...
    # ⭐ 不在这里创建 parser，在 pad-added 中动态创建
    ...
    if not all([source, qtdemux, decoder, streammux,
                pgie1, pgie2, pgie3, nvosd, nvvidconv, capsfilter, 
                encoder, h264parser_enc, qtmux, sink]):
    ...
    print(f"  Mode: SERIAL with auto-format detection\n")

    # ==================== 添加到 pipeline ====================
    # 注意：parser 会在 pad-added 中动态添加
    for elem in [source, qtdemux, decoder, streammux,
                 pgie1, pgie2, pgie3, nvosd, nvvidconv, capsfilter, 
                 encoder, h264parser_enc, qtmux, sink]:
        pipeline.add(elem)
    ...
    def qtdemux_pad_added(demux, src_pad):
        ...
        # 11. 串行链接三个模型
        try:
            if not streammux.link(pgie1):
                print(f"  [ERROR] Failed to link streammux -> pgie1")
                return
            print(f"  [OK] streammux -> pgie1")
            
            if not pgie1.link(pgie2):
                print(f"  [ERROR] Failed to link pgie1 -> pgie2")
                return
            print(f"  [OK] pgie1 -> pgie2")
            
            if not pgie2.link(pgie3):
                print(f"  [ERROR] Failed to link pgie2 -> pgie3")
                return
            print(f"  [OK] pgie2 -> pgie3")
            
            if not pgie3.link(nvosd):
                print(f"  [ERROR] Failed to link pgie3 -> nvosd")
                return
            print(f"  [OK] pgie3 -> nvosd")
            ...
            qtmux.link(sink)
            print(f"  [OK] Output chain: nvosd -> encoder -> sink")
            
            print(f"\n  [SUCCESS] SERIAL pipeline fully linked with {parser_name}!")
            print(f"  Architecture: {parser_name} -> pgie1 -> pgie2 -> pgie3 -> osd")
